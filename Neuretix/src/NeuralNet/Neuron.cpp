...
Neuron* Neuron::FindNeuronByID(int id, std::vector<Neuron*>* neuronList) //returns a pointer to the neuron with ID==ID in a group
{
    if(neuronList!=NULL)
    {
		for(int n_c = 0; n_c<neuronList->size(); n_c++)
		{
			Neuron* n = neuronList->at(n_c);
			if(n->id == id) return n;
		}
    }
	return NULL;
